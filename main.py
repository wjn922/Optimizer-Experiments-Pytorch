...
parser.add_argument('--batch_size', default=1024, type=int, metavar='N', help='train batchsize')
args = parser.parse_args()

state = {k: v for k, v in args._get_kwargs()}

############## change name ##################
fold = 'SGD1118_single'   # for experiments
writer = SummaryWriter(logdir='runs/SGD1118_single')
#############################################
...
if not os.path.exists(("runs/" + fold)):
    os.makedirs("runs/" + fold)
...
def main():
    ...
    if args.resume:
        ...
        filepath = os.path.join(checkpoints_path, fold + os.sep + 'checkpoint.pth.tar')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        }, is_best=is_best, checkpoints=checkpoints_path, fold=fold)
...
def train(trainloader, net, criterion, optimizer, epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        # if needed, log the weights
        # for name, params in net.named_parameters():
            # writer.add_histogram(name, params, epoch)
